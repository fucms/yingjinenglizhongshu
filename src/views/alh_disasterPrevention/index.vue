    ...
    <div class="filter-container">
      ...
      <el-form :inline="true" :model="listQuery" class="demo-form-inline">
        <el-form-item label="年份">
          <el-input v-model="listQuery.filter1" placeholder="请输入年份" />
        </el-form-item>
        <el-form-item label="灾害类型">
          <el-input v-model="listQuery.filter2" placeholder="请输入灾害类型" />
        </el-form-item>
        <el-form-item label="灾害等级">
          <el-input v-model="listQuery.filter3" placeholder="请输入灾害等级" />
        </el-form-item>
        <el-form-item>
      ...
      >
        ...
        <el-table-column label="年份" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code1 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="灾害类型" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code2 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="灾害等级" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code3 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="资源分布" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code4 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="应急响应策略" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code5 }}</span>
          </template>
        </el-table-column>
