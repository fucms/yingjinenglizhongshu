    ...
    <div class="filter-container">
      ...
      <el-form :inline="true" :model="listQuery" class="demo-form-inline">
        <el-form-item label="设备编号">
          <el-input v-model="listQuery.filter1" placeholder="请输入设备编号" />
        </el-form-item>
        <el-form-item label="设备名称">
          <el-input v-model="listQuery.filter2" placeholder="请输入设备名称" />
        </el-form-item>
        <el-form-item>
      ...
      >
        ...
        <el-table-column label="告警等级" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code3 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="告警详情" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code4 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="所属部门" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code5 }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="状态" prop="code" align="center">
          <template slot-scope="{ row }">
            <el-tag :type="row.type1 == 1? 'success' : 'danger'">{{ row.type1 == 1? '正常' : '不正常' }}</el-tag>
          </template>
        </el-table-column>
  ...
  methods: {
    ...
    edit(form) {
      this.list.forEach(item => {
        if (item.code1 === form.customerCode1) {
          item.code2 = form.customerCode2
          item.code3 = form.customerCode3
          item.code4 = form.customerCode4
          item.code5 = form.customerCode5
          item.code6 = form.customerCode6
          item.type1 = form.type1
        }
      })
    },
    handleFilter() { },
