    ...
    <div class="filter-container">
      ...
      <el-form :inline="true" :model="listQuery" class="demo-form-inline">
        <el-form-item label="事件名称">
          <el-input v-model="listQuery.filter1" placeholder="请输入事件名称" />
        </el-form-item>
        <el-form-item label="事件类型">
          <el-input v-model="listQuery.filter2" placeholder="请输入事件类型" />
        </el-form-item>
        <el-form-item label="发生地">
          <el-input v-model="listQuery.filter3" placeholder="请输入发生地" />
        </el-form-item>
        <el-form-item>
      ...
      >
        ...
        <el-table-column label="事件名称" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code1 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="事件类型" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code2 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="发生地" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code3 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="发生时间" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code4 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="应急资源分布" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code5 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="应急响应情况" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code6 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="气象" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code7 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="人口" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code8 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="交通路况" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code9 }}</span>
          </template>
        </el-table-column>
